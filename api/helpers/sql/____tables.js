...
GLOBAL.tables = {
  ...
  constellations: {
    ...
    new: function(constellation) {      
      return knex.insert(without_fields(constellation, ["id", "limits", "hill_sphere", "stars"]))
             .into(tables.celestial_objects.name)
             .then(function(ids) { 
                constellation.id = ids[0];
                return tables.celestial_objects.get(ids[0]);
              })
             .then(function(data) { return without_nulls(data, true) })
             .then(() => knex.insert(without_fields(constellation, ["name", "discoverer", "discovery_date", 
                                          "discovery_place", "age", "volume", "mass", "radius", "surface_area", 
                                          "mean_density", "whose_satellite", "stars"]))
  ...
  stars: {
    ...
    new: function(constellationId, star) {            
      ...
      return knex.insert(without_fields(star, ["id", "right_ascension", "declination", "apparent_magnitude", 
                              "distance", "radial_velocity", "luminosity", "effective_temperature", 
                              "constellation_id", "planetary_system_id"]))
             .into(tables.celestial_objects.name)
             .then(function(ids) { 
                star.id = ids[0];
                return tables.celestial_objects.get(ids[0]);
             ...
             .then(function(data) { return without_nulls(data, true) })
             .then(() => knex.insert(without_fields(star, ["name", "discoverer", "discovery_date", 
                                          "discovery_place", "age", "volume", "mass", "radius", "surface_area", 
                                          "mean_density", "whose_satellite"]))
                             .into(tables.stars.name)
                             .then(function(ids) { return tables.stars.get(constellationId, ids[0]) })
                             .then(function(data) { return without_nulls(data, true) })                
              )      
    },
    ...
    get: function(constellationId, starId) {
      ...
      if (constellationId) {
        if (starId) {
          query = knex.first()
                      .from(tables.celestial_objects.name)
                      .innerJoin(tables.stars.name, tables.celestial_objects.name + ".id", tables.stars.name + ".id")
                      .where(tables.stars.name + ".id", starId);
        } 
        else {
          query = knex.select()
                      .from(tables.celestial_objects.name)
    ...
    get: function (constellationId, nebulaId) {
      ...
      if (constellationId) {
        if (nebulaId) {
          query = knex.first()
                      .from(tables.celestial_objects.name)
                      .innerJoin(tables.nebulas.name, tables.celestial_objects.name + ".id", 
                                 tables.nebulas.name + ".id")
                      .where("id", nebulaId);
        }
        else {
          query = knex.select()
  ...
  galaxies: {
    ...
    new: function(galaxy) {      
      return knex.insert(without_fields(galaxy, ["name", "discoverer", "discovery_date", "discovery_place", "age", 
                              "volume", "mass", "radius", "surface_area", "mean_density", "whose_satellite"]))
                .into(tables.galaxies.name)
                .then(function(ids) { return tables.galaxies.get(ids[0]) })
                .then(function(data) { return without_nulls(data, true) })
                .then(function(response) { 
                    return knex.insert(without_fields(galaxy, ["hill_sphere", "snow_line", "visible_dimensions", 
                                            "surface_brightness", "distance", "radial_velocity"]))
                               .into(tables.celestial_objects.name)
                               .then(function(ids) { return tables.galaxies.get(ids[0]) }) 
                               .then(function(data) { return without_nulls(data, true) })
                });  
    ...
    get: function (id) {
      ...
      if (id) {
        query = knex.first()
                    .from(tables.celestial_objects.name)
                    .innerJoin(tables.galaxies.name, tables.celestial_objects.name + ".id", 
                               tables.galaxies.name + ".id")
                    .where(tables.galaxies.name + ".id", id);
      }
      else {
        query = knex.select()
                    .from(tables.celestial_objects.name)
  ...
  planetary_systems: {
    ...
    get: function (id) {
      ...
      if (id) {
        query = knex.first()
                    .from(tables.celestial_objects.name)
                    .innerJoin(tables.planetary_systems.name, tables.celestial_objects.name + ".id", 
                               tables.planetary_systems.name + ".id")
                    .where(tables.planetary_systems.name + ".id", id);
      }
      else {
        query = knex.select()
                    .from(tables.celestial_objects.name)
    ...
    get: function (planetarySystemId, planetId) {
      ...
      if (planetarySystemId) {
        if (planetId) {
          query = knex.first()
                      .from(tables.celestial_objects.name)
                      .innerJoin(tables.planetoids.name, tables.celestial_objects.name + ".id", 
                                 tables.planetoids.name + ".id")
                      .innerJoin(tables.planets.name, tables.planetoids.name + ".id", tables.planets.name + ".id")
                      .where("id", planetId);
        } 
        else {
          query = knex.select()
      ...
      if (planetarySystemId) {
        if (dwarfPlanetId) {
          query = knex.first()
                      ...
                      .innerJoin(tables.planetoids.name, tables.celestial_objects.name + ".id", 
                                 tables.planetoids.name + ".id")
                      .innerJoin(tables.dwarf_planets.name, tables.planetoids.name + ".id", 
                                 tables.dwarf_planets.name + ".id")
                      .where("id", dwarfPlanetId);
        } 
        else {
          query = knex.select()
                      .from(tables.celestial_objects.name)
      ...
      if (planetarySystemId) {
        if (asteroidId) {
          query = knex.first()
                      ...
                      .innerJoin(tables.planetoids.name, tables.celestial_objects.name + ".id", 
                                 tables.planetoids.name + ".id")
                      .innerJoin(tables.asteroids.name, tables.planetoids.name + ".id", tables.asteroids.name + ".id")
                      .where("id", asteroidId);
        }
        else {
          query = knex.select()
      ...
      if (planetarySystemId) {
        if (cometId) {
          query = knex.first()
                      ...
                      .innerJoin(tables.planetoids.name, tables.celestial_objects.name + ".id", 
                                 tables.planetoids.name + ".id")
                      .innerJoin(tables.comets.name, tables.planetoids.name + ".id", tables.comets.name + ".id")
                      .where("id", cometId);
        }
        else {
          query = knex.select()
      ...
      if (planetId) {
        if (satelliteId) {
          query = knex.first()
                      ...
                      .innerJoin(tables.planetoids.name, tables.celestial_objects.name + ".id", 
                                 tables.planetoids.name + ".id")
                      .innerJoin(tables.satellites.name, tables.planetoids.name + ".id", 
                                 tables.satellites.name + ".id")
        ...
        query = knex.select()
                    ...
                    .innerJoin(tables.planetoids.name, tables.celestial_objects.name + ".id", 
                               tables.planetoids.name + ".id")
                    .innerJoin(tables.satellites.name, tables.planetoids.name + ".id", tables.satellites.name + ".id");
      }
    }
  }
}
